...
func NewSimpleUpgrader(ctx context.Context, log *logrus.Entry, cs *api.OpenShiftManagedCluster, initializeStorageClients, disableKeepAlives bool, testConfig api.TestConfig) (Upgrader, error) {
	...
	kubeclient, err := kubeclient.NewKubeclient(log, cs.Config.AdminKubeconfig, disableKeepAlives, cs.Location, cs.Properties.NetworkProfile.PrivateEndpoint, testConfig)
	if err != nil {
		return nil, err
	}
...
func (u *Upgrade) ReloadKubeClient(disableKeepAlives bool) error {
	kubeclient, err := kubeclient.NewKubeclient(u.Log, u.Cs.Config.AdminKubeconfig, disableKeepAlives, u.Cs.Location, u.Cs.Properties.NetworkProfile.PrivateEndpoint, u.TestConfig)
	if err != nil {
		return err
	}
